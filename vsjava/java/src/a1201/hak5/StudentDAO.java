...
public class StudentDAO {
    ...
    private void delete(int index) {
        slist.remove(index);
        // remove = ArrayList에서 한줄 삭제(인덱스번호로)
    }
    private StudentDTO select(int index) {
        return slist.get(index); // 해당 인덱스 객체 전체
    }
    ...
    public void userDelete() {
        ...
        private int searchIndex() {
            ...
            System.out.print(">>");
            String name = sc.next();
            for(int i=0; i < slist.size(); i++){ // slist라는 arrayList 배열이름과 입력한 이름비교
        ...
        public void userSelect() {
            ...
            if(index == -1){
                ...
                System.out.println(s); // to String이 생략 되어있는 형태 (DTO에 만들어 놓음)
            }
        }
