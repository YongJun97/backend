$(function(){
    ...
    // pager li 의 클릭기능을 키고 click_slide라는 함수를 실행

    function click_slide(){
        $('.pager li').off('click');
        // 실행이 중복되지 않도록 클릭기능 끄기

        i=$(this).index();
        //전역변수 i에 클릭한 li의 인덱스 번호를 저장
        if(i > k){
            $('.imgs li').eq(k).css('left','0').animate({'left':'-100%'},500)
            $('.imgs li').eq(i).css('left','100%').animate({'left':'0%'},500,function(){
                $('.pager li').on('click',click_slide);
            });
            //슬라이드가 끝난후에 동작 - 콜백함수
        }else if(i < k){
    ...
    function timer(){
       repeat = setInterval(function(){
		...
		$('.imgs li').eq(i-1).css('left','0').animate({'left':'-100%'},500);
        $('.imgs li').eq(i).css('left','100%').animate({'left':'0%'},500);
